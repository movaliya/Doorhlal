...
- (void)viewDidLoad {
    ...
    [KmyappDelegate SetimageinAndPrefixTextfield:self.UserPhoneNo_txt:@"IconPhone"];
...
- (IBAction)NextBtn_action:(id)sender
{
    ...
    if (AddressArr.count==0)
    {
        ...
        {
           if (![AppDelegate myMobileNumberValidate:_UserPhoneNo_txt.text])
            {
...
-(void)SendBillDetail
{
    ...
    if (AddressArr.count==0)
    {
        ...
        NSString *callingCode=[NSString stringWithFormat:@"+%@",[[filtered valueForKey:@"callingCode"] objectAtIndex:0]];
        NSString *phoneStr=[NSString stringWithFormat:@"%@%@",callingCode,_UserPhoneNo_txt.text];
        ...
        [dictParams setObject:_State_TXT.text  forKey:@"u_state"];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    {
        if (_UserPhoneNo_txt.text.length >= MAX_LENGTH && range.length == 0)
        {
